...
-- here a am checking all table
 
select * from countries;   -- country table
select * from departments;   -- department table
select * from employees;     -- employee table
select * from job_history;   -- job history table
select * from jobs;          -- job table
select * from locations;     -- location table
select * from regions;       -- region table
...
-----------------------------------------------------------
 ...
 select salary  from employees;   -- all saleries
 
 
 -- Q2 Write a SQL query to find the unique designations of the employees. Return job name.
 
 select DISTINCT JOB_TITLE from jobs;   -- job name
 
 
 -- Q3 write a SQL query to list the employees’ name, increased their salary by 15%, and expressed as number of Dollars.
 
 SET SQL_SAFE_UPDATES=0;       -- setting to do update and deletion.
 select * from employees;       -- table of employee
 update employees set SALARY=(select SALARY + (select SALARY*.15))    -- updating salary with 15%
 
 select FIRST_NAME, SALARY from employees;      -- first name with salary increased by 15%
 
 
 -- Q4  Write a SQL query to list the employee's name and job name as a format of "Employee & Job".
 
 select * from employees;        -- employee table
 select * from job;              -- jobs table
 
 select FIRST_NAME, JOB_TITLE As "Employee & job"  
from employees, jobs;
 
 -- Q5 Write a SQL query to find those employees with hire date in the format like February 22, 1991. 
 -- Return employee ID, employee name, salary, hire date.
 
select * from employees;        -- employees table
desc employees;                  -- describe table Dtype
alter table employees            -- changing Dtype
modify HIRE_DATE varchar(255);    
SET SQL_SAFE_UPDATES = 0;       -- changing date formating
update employees                -- updating date format
set HIRE_DATE=date_format(HIRE_DATE,"%b/%d/%Y");

select EMPLOYEE_ID, FIRST_NAME, HIRE_DATE, SALARY from employees;       -- making queries
...
select DISTINCT(DEPARTMENT_ID), JOB_TITLE from employees, jobs;
...
-- here is no 'ANALYST' post in job table as well as employees table

select  * from employees;         -- employee table
select avg(SALARY) As "avg salary"   -- avg salary
from employees                     -- job id
